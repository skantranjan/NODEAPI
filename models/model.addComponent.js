...
async function insertComponentAuditLog(data) {
  try {
    console.log('🔍 === AUDIT LOG INSERTION DEBUG ===');
    console.log('📊 Incoming audit data:', JSON.stringify(data, null, 2));
    
    // Debug: Check for array/object fields
    Object.keys(data).forEach(key => {
      if (Array.isArray(data[key])) {
        console.log(`⚠️ ARRAY FIELD DETECTED: ${key} =`, data[key]);
      } else if (typeof data[key] === 'object' && data[key] !== null) {
        console.log(`⚠️ OBJECT FIELD DETECTED: ${key} =`, data[key]);
      }
    });
    ...
    const safeExtractValue = (value, fieldName) => {
      ...
      if (Array.isArray(value)) {
        console.log(`⚠️ Warning: Field ${fieldName} is an array, using first value:`, value);
        ...
        if (['component_id', 'material_type_id', 'component_uom_id', 'component_base_uom_id', 
          ...
          if (isNaN(intValue)) {
            console.log(`⚠️ Warning: Field ${fieldName} array value cannot be converted to integer, using null`);
            return null;
          }
          return intValue;
        }
      ...
      if (typeof value === 'object') {
        console.log(`⚠️ Warning: Field ${fieldName} is an object, converting to string:`, value);
      ...
      if (['component_id', 'material_type_id', 'component_uom_id', 'component_base_uom_id', 
        ...
        if (isNaN(intValue)) {
          console.log(`⚠️ Warning: Field ${fieldName} value cannot be converted to integer:`, value);
          return null;
        }
        return intValue;
      }
    ...
    const values = [
      ...
      safeExtractValue(data.component_valid_from, 'component_valid_from'),
      safeExtractValue(data.component_valid_to, 'component_valid_to'),
    ...
    console.log('🔍 Audit log values being inserted:', values);
