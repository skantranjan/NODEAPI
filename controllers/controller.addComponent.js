...
async function addComponentController(request, reply) {
  try {
    ...
    if (componentData.periods && !componentData.year) {
      ...
      console.log(`📅 Setting year field to match periods: ${componentData.year}`);
    } else if (componentData.year && !componentData.periods) {
      componentData.periods = componentData.year;
      console.log(`📅 Setting periods field to match year: ${componentData.periods}`);
      ...
      console.log(`⚠️ Year (${componentData.year}) and periods (${componentData.periods}) are different. Using periods value for both.`);
      componentData.year = componentData.periods;
    }

    // Log the synchronized values
    console.log(`📅 Final synchronized values - Year: ${componentData.year}, Periods: ${componentData.periods}`);
